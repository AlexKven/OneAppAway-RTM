...
namespace OneAppAway._1_1.Data
{
    public class TransitStopSqlProvider : SqlProviderBase<TransitStop>
    {
        ...
        public override IEnumerable<TransitStop> Select(SQLiteConnection connection, string conditions = null)
        {
            var results = base.Select(connection, conditions);
            return results.Select(item =>
            {
                item.Children = ExecuteSQL(connection, $"select ID from {TableName} where Parent = '{item.ID}';").GetColumn(0);
                return item;
            });
        }
    }
